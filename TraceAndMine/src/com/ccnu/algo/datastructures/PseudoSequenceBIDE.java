...
//					if(listPositions.size()+ listPositionsTotal.size()	== getItemOccurencesTotalCount(prefix))  // si on a trouv�tout le pr閒ix
//					{
...
//	}
	 ...
	 * @param i : le i鑝e 閘閙ent du pr閒ixe.
	 ...
	 */
	public PseudoSequenceBIDE getIthMaximumPeriodOfAPrefix(List<Itemset> prefix, int i){
		...
		System.out.println("1"+lastInstancePair);
...
// y
	...
	protected  PseudoSequencePair getLastInstanceOfPrefixSequenceNEW(List<Itemset> prefix, int i){
		...
		for(int j = size() -1; j>=0; j--){
			System.out.println("C"+prefix+""+i+"j"+j);
			...
			if(allMatched) {
				...
				if(prefixItemsetPosition == -1) {
					PseudoSequencePair p=new PseudoSequencePair(this, listPositions);
					System.out.println("return "+p.list.size());
...
//			pseudo.toString();
	 ...
	 * @param i : the i鑝e element of the prefix.
